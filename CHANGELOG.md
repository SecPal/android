...
- Domain-policy storage-key exemptions now use a fail-closed fixpoint proof for initialized keys and reachable, zero-argument named helpers, count at most eight aggregate live helper calls across each proof and its prefixes, ignore erased type references and proven dormant functions, methods, or accessors, and reject unproven prefixes, longer paths, exports, asynchronous or parameterized helpers, generators, decorators, optional calls, deferred calls, and escaped flows.
