...
- Added a credential-free Fastlane withdrawal lane that serializes direct deploy and withdrawal operations with a shared remote lock, requires the explicit version list to cover every currently advertised release, recovers interrupted metadata transactions without rolling back a committed unavailable state, terminates after signal-driven rollback, remotely resolves and rejects overlapping or symlinked public quarantine paths, reserves a collision-free quarantine transaction before changing metadata, publishes non-downloadable unavailable metadata for Stable, the Stable aliases, and Beta, and moves the affected Latest, interrupted-publication remnants, and explicitly selected versioned APK/checksum artifacts into a recoverable non-public quarantine without deriving signing identity from an absent APK or restoring schema-3 compatibility (issue `#434`, part of `SecPal/.github#590`).
