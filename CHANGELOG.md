...
- Android push registration now permanently disables itself with a structured non-retryable error when secure Web Crypto UUID generation is unavailable in the injected WebView, instead of dropping the registration attempt into the generic sync warning path, and adds focused Vitest coverage for issue `#244`.
