...
- Added a credential-free Fastlane withdrawal lane that recovers interrupted metadata transactions, publishes non-downloadable unavailable metadata for Stable, the Stable aliases, and Beta, and moves the affected Latest and explicitly selected versioned APK/checksum artifacts into a recoverable non-public quarantine without deriving signing identity from an absent APK or restoring schema-3 compatibility (issue `#434`, part of `SecPal/.github#590`).
