...
- Domain-policy storage-key exemptions now prove that initialization precedes up to eight direct, zero-argument named helper calls, including the top-level live invocation, while rejecting longer, exported, asynchronous, parameterized, generator, decorated, optional, deferred, and otherwise escaped helper flows.
