...
- Replaced regex-only browser-storage key exemptions in domain-policy validation with TypeScript syntax-tree, symbol, compiler reachability, and storage-mutation analysis, including scope, shadowing, executed initializers and methods, complete storage calls, deferred, conditional, suspended, and abruptly completed execution, static computed access, regex literals, template interpolation handling, self-scanning policy sources, and actionable dependency errors.
