...
- Established the release-evidence gate for retiring Android notification schema `3`: fresh and native-restored runtime regression coverage now proves strict integer schema `4`, the unused pre-production Direct and Google Play Internal schema-3 history is recorded without requiring unavailable retired binaries, and APK-level evidence for the first clean release is required before selecting and deploying the production minimum-supported floor without changing channel semantics.
