...
- Android runtime bootstrap now requests an FCM registration token from the deployment-scoped native Firebase runtime, retains named-app token and error events on the native auth bridge for later device-registration handling, and adds focused Java coverage for issue `#241`.
