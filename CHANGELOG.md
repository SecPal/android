...
- Android push token retention now persists trusted runtime FCM tokens into logout-safe browser storage as soon as the canonical runtime origin is known, rehydrates `__SecPalAndroidPushSyncState.currentToken` after the `/` -> `/login` recovery reload, and adds focused Vitest coverage for both logout-cleared storage and early-token-before-bootstrap-restore timing in issue `#248`.
