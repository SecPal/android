...
- Loosened the Samsung Knox hard-key broadcast gate from `isKioskActive()` to `isDeviceOwner()` so XCover hardware-button events are received in all device-owner scenarios, not only when kiosk mode is active.
- Device-owner policy now unconditionally disables the platform keyguard via `setKeyguardDisabled` so hardware-button presses can wake the device and launch SecPal without requiring an unlock step.

### Added

- Android hardware-button emergency bridge: the generic runtime now distinguishes short and long presses for eligible non-system foreground `KeyEvent`s, while managed Samsung devices also wire an optional Knox hard-key broadcast receiver behind a dedicated vendor adapter; the native plus injected `SecPalEnterpriseBridge` tags each event origin, opens `/profile` on short press, opens `/about` after holds of at least five seconds, now prefers Samsung Knox hard-key report broadcasts so dedicated XCover-style buttons can keep that short-versus-long distinction even when SecPal is backgrounded or waking from the lockscreen, and falls back to the raw Samsung hard-key press broadcast so SecPal still returns to the foreground when report delivery is unavailable
- Samsung XCover launch-alias fallback for devices without premium Knox custom permission grants: the Android wrapper now exposes dedicated `ProfileHardwareTriggerActivity` and `AboutHardwareTriggerActivity` aliases so Samsung's built-in XCover key customization can still launch SecPal globally into `/profile` or `/about` even when `KNOX_CUSTOM_PROKIOSK` or `KNOX_CUSTOM_SYSTEM` APIs remain unavailable despite device-owner provisioning
- Samsung XCover auto-configuration fallback: the Android wrapper now makes those hardware-trigger aliases launcher-capable and best-effort syncs Samsung's `dedicated_app_xcover`, `short_press_app`, and `long_press_app` secure settings through the device-owner policy channel, so managed Samsung test devices no longer depend on a manual settings-app remap to point short and long hardware-key presses at SecPal even though direct app-level `WRITE_SETTINGS` writes are rejected on current XCover firmware
