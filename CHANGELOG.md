...
- Enforced schema `4` as the only Android runtime-bootstrap contract: shared frontend discovery requires strict integer schema `4`, the injected bridge always emits integer schema `4` for notification registration after fresh setup or native restoration, persisted schema and minimum-version/build markers cannot override or gate that value, and signed APK/AAB builds fail before upload unless the packaged WebView bridge contains exactly one canonical, executable schema-4 registration path; artifact inspection now rejects commented or non-canonical bridge tags, reports missing or corrupt archives accurately, preserves archive-fixture process failures, and removes the superseded compatibility wording without changing authentication, push lifecycle, or Device Owner/profile-owner behavior (issue `#432`, part of `SecPal/.github#590`).
