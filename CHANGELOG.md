...
- Removed the retired frontend-issued Android enrollment bootstrap exchange, its persisted state and enterprise bridge payload, and the obsolete `managedAndroidEnrollment` runtime-bootstrap compatibility flag; app startup now deletes any encrypted token and derived enrollment state left by an older installation without reading it, while independent Device Owner/profile-owner policy and Stable/Beta distribution behavior remain unchanged.
