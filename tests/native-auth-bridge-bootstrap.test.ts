...
describe("native auth bridge bootstrap injection", () => {
  ...
  async function createAndroidPushLifecycleSandbox(
    options: {
      ...
      runtimeBootstrap?: ReturnType<
        typeof createCustomerAndroidPushBootstrap
      > | null;
    ...
    const runtimeBootstrap =
      options.runtimeBootstrap === undefined
    ...
    const deferredLifecycleHandles: Array<() => void> = [];
    const plugin = {
      login: vi.fn().mockResolvedValue({ user: { id: 7 } }),
      logout: vi.fn().mockResolvedValue(undefined),
      getCurrentUser: vi.fn().mockResolvedValue({ id: 7 }),
      ...
      getRuntimeBootstrap: vi.fn().mockResolvedValue({
        configured: runtimeBootstrap !== null,
      ...
      addListener: vi.fn(
        (
          ...
          return new Promise<typeof handle>((resolveHandle) => {
            deferredLifecycleHandles.push(() => resolveHandle(handle));
          });
        }
      ),
    };
    ...
    return {
      ...
      resolveLifecycleHandle: () => {
        for (const resolveHandle of deferredLifecycleHandles) {
          resolveHandle();
        }
      },
      sandbox,
  ...
  it("invalidates every legacy browser push identity without bridging it", async () => {
    ...
    expect(plugin).not.toHaveProperty("retainLegacyAndroidPushInstallation");
  ...
  it("removes legacy push state for every runtime while preserving other state", async () => {
    ...
    expect(localStorage.getItem(otherInstallationKey)).toBeNull();
    expect(sessionStorage.getItem(currentInstallationKey)).toBeNull();
    expect(
  ...
  it("removes legacy installation state when no runtime is configured", async () => {
    const legacyInstallationId = "11111111-1111-4111-8111-111111111111";
    const localStorage = createMockStorage({
      "secpal-android-push-installation:https%3A%2F%2Fcustomer-api.example":
        legacyInstallationId,
  ...
  it("removes the native auth lifecycle listener when the WebView page is hidden", async () => {
    ...
    expect(
      handles.every((handle) => handle.remove.mock.calls.length === 0)
    ).toBe(true);
    ...
    expect(
      handles.every((handle) => handle.remove.mock.calls.length === 1)
    ).toBe(true);
  });
  ...
  it("invalidates the WebView session on the abstract native event", async () => {
    const { listeners, sandbox } = await createAndroidPushLifecycleSandbox();
    const authState = sandbox.__SecPalNativeAuthState as { active: boolean };
    const logoutEvents = vi.fn();
    (
      sandbox.addEventListener as (
        eventName: string,
        listener: (event: { type: string }) => void
      ) => void
    )("secpal:native-auth-logout", logoutEvents);
    authState.active = true;

    listeners.nativeAuthSessionInvalidated?.[0]?.({
      reason: "AUTHENTICATION_REJECTED",
    });

    expect(authState.active).toBe(false);
    expect(logoutEvents).toHaveBeenCalledOnce();
  });
