...
describe("native auth bridge bootstrap injection", () => {
  ...
  it("installs the native bridge, keeps vault wrapper methods off the bootstrap bridge, and routes authenticated /v1/ fetch traffic through the native plugin", async () => {
    const { buildNativeAuthBridgeBootstrapScript } = await loadInjectorModule();
    const plugin = {
      login: vi.fn().mockResolvedValue({ user: { id: 7 } }),
    ...
    const vaultBridge = bridge as Record<string, unknown>;
    ...
    expect("isVaultDeviceBoundWrapperAvailable" in vaultBridge).toBe(false);
    expect("wrapVaultRootKey" in vaultBridge).toBe(false);
    expect("unwrapVaultRootKey" in vaultBridge).toBe(false);
