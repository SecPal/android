...
describe("native auth bridge bootstrap injection", () => {
  ...
  it("keeps raw Android push identities native-only and purges legacy WebView copies", async () => {
    const { buildNativeAuthBridgeBootstrapScript } = await loadInjectorModule();
    const bootstrap = buildNativeAuthBridgeBootstrapScript(
      runtimeBootstrapPlaceholderOrigin
    );
    const tokenStorageKey =
      ...
      encodeURIComponent("https://api.secpal.dev");
    const installationStorageKey =
      "secpal-android-push-installation:" +
      encodeURIComponent("https://api.secpal.dev");
    const localStorage = createMockStorage({
      [tokenStorageKey]:
        "legacy-fcm-token-1234567890abcdefghijklmnopqrstuvwxyz",
      [installationStorageKey]: "11111111-1111-4111-8111-111111111111",
    });
    const sessionStorage = createMockStorage({
      [tokenStorageKey]: "legacy-session-fcm-token-1234567890abcdefghijk",
    });
    ...
    runNativeBridgeInContext(bootstrap, sandbox);
    await flushMicrotasks();

    expect(localStorage.getItem(tokenStorageKey)).toBeNull();
    expect(localStorage.getItem(installationStorageKey)).toBeNull();
    expect(sessionStorage.getItem(tokenStorageKey)).toBeNull();
    expect(bootstrap).not.toContain("androidPushTokenReceived");
    expect(bootstrap).not.toContain("notification-installations");
    expect(bootstrap).not.toContain("__SecPalAndroidPushSyncState");
    expect(bootstrap).not.toContain("push_token");
  ...
  it("dispatches a native logout event after the bridge completes logout", async () => {
    const { bridge, sandbox } = await createNativeAuthLifecycleSandbox();
