...
`withdraw_direct_apks` first recovers an interrupted metadata transaction, preserving a committed unavailable state even when marker cleanup was interrupted, publishes non-downloadable unavailable metadata for Stable, the Stable aliases, and Beta, and then moves the public Latest APK/checksum files, interrupted-publication remnants, and the comma-separated `SECPAL_ANDROID_WITHDRAW_VERSIONS` release directories outside the public artifact root. The recoverable quarantine defaults to the sibling `android-artifacts-withdrawn` directory and can be overridden with an absolute `SECPAL_ANDROID_DIRECT_WITHDRAWAL_ROOT`. The lane resolves both roots on the SSH host, rejects overlap and symlink traversal into the public tree, and refuses to reuse an existing quarantine transaction. It needs SSH access but deliberately does not load signing credentials or inspect an APK.
