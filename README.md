...
The native shell owns the FCM token, installation UUID, timestamps, and server-registration binding in Android Keystore-encrypted storage. WebView JavaScript receives only abstract registration state and can request an input-free retry; it never receives the identifiers or constructs the server payload. Startup binds protected identity before Firebase token retrieval, and every foreground resume re-queries the named runtime so rotation is detected without waiting for a process restart. After native login, Android registers `PUT /v1/me/notification-installations/{installationId}` on the canonical API origin, rotates that same binding idempotently, and revokes it on logout or `Log out and switch instance`. Runtime or metadata changes invalidate the old native binding before a new deployment can use it, while a `409` registration rejection exposes a non-identifier `reconfiguration_required` state for the intentional runtime-reset flow.

For operator validation on a real device, confirm the app binds to the intended customer instance, login creates the notification-installation registration on the customer API host, logout or instance reset revokes that registration, and no push traffic falls back to any SecPal-owned API or legacy Firebase setup.
...
The script keeps the configured runtime (or completes discovery first when needed), fills the React-controlled login form through the DOM, waits for native auth completion, and then verifies the abstract native Android push state reaches `registered` without reading a token or installation ID. Override `SECPAL_RUNTIME_URL`, `SECPAL_WEBVIEW_DEVTOOLS_URL`, or `SECPAL_WEBVIEW_TARGET_PATTERN` if your test target differs. If the app restarts and the `webview_devtools_remote_<pid>` socket changes, redo the `adb forward` step before rerunning the smoke command. The smoke aborts when native auth or registration already completed before the DOM login step, so a green run proves a fresh login and native registration transition instead of reusing stale session state.
