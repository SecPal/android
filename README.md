...
For hardware-button emergency flows, SecPal keeps the core bridge vendor-neutral and adds Samsung support only through dedicated native adapters. In the generic Android path, SecPal still reacts only to in-app hardware button events, opens `/profile` on a short press, and opens `/about` after a hold of at least five seconds. On managed Samsung devices, SecPal first tries the Knox hard-key broadcast path whenever the app is device owner; if the device does not grant the premium Knox custom permissions required for that route, SecPal can still be launched globally through Samsung's built-in XCover key customization by mapping the short press to `app.secpal.ProfileHardwareTriggerActivity` and the long press to `app.secpal.AboutHardwareTriggerActivity`. Those aliases are now launcher-capable so Samsung's key-customization layer can target them directly, and SecPal also best-effort synchronizes the related Samsung secure settings through its device-owner policy channel instead of relying on a manual Settings-app remap. The aliases feed back into the same short-/long-press bridge events, attempt to wake past the lockscreen, open `/profile` for short launches, and open `/about` for long launches. Other OEMs continue to use the generic fallback path because Android does not expose a portable equivalent for vendor-wide global hard-key capture.
