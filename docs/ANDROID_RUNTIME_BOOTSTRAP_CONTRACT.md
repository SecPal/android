...
| `SecPalNativeAuthBridge.clearRuntimeBootstrap()`                  | Injected bridge delegates to `SecPalNativeAuth.confirmRuntimeReset()`, which identifies the native canonical origin before clearing persistence and tenant-bound credentials. Browser state is cleared only after native success.                                                           | Required for an explicit user-approved instance reset; cancellation or native failure preserves the current frontend runtime and tenant state. |
