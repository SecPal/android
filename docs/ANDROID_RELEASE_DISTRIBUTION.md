...
Direct Stable/Beta deploys and withdrawals hold the same canonical remote artifact-root lock from deploy version selection through publication or quarantine. A second mutation fails before changing remote state. If an ungraceful process or host termination leaves the empty `${SECPAL_ANDROID_DIRECT_ROOT}.release.lock` directory behind, verify that no direct release mutation is still running and remove only that empty directory with `rmdir` before retrying.
When no supported signed replacement is available, `fastlane android withdraw_direct_apks` requires the comma-separated `SECPAL_ANDROID_WITHDRAW_VERSIONS` value to name at least one versioned release and cover every version currently advertised by Stable, its `/android/` aliases, or Beta. It recovers any interrupted metadata replacement and reserves a new quarantine transaction before changing public state. The lane first moves every public Latest APK/checksum, any interrupted-publication Latest staging/backup files, and each selected release directory into the prepared recoverable quarantine outside the public artifact root. Only after that completes does it change the channel documents to `release_available: false`, so an artifact-move failure cannot leave unavailable metadata alongside public schema-3 files. A transaction commit marker prevents later cleanup interruption from rolling a committed unavailable document back to an older available release, while HUP, INT, and TERM roll back an uncommitted metadata replacement and terminate non-zero. Unavailable metadata sets both the latest APK URL and checksum URL to `null`. The quarantine defaults to the sibling `android-artifacts-withdrawn` directory; set an absolute `SECPAL_ANDROID_DIRECT_WITHDRAWAL_ROOT` to use another non-public path. Before changing artifacts or metadata, the lane resolves both roots on the SSH host and rejects incomplete version coverage, containment, symlink traversal into the public tree, or an existing quarantine transaction. The lane requires only the configured SSH release host and does not load or synthesize signing credentials.
