...
When no supported signed replacement is available, `fastlane android withdraw_direct_apks` first recovers any interrupted metadata replacement and then changes the Stable channel, its `/android/` aliases, and the Beta channel metadata to `release_available: false`. A transaction commit marker prevents later cleanup interruption from rolling a committed unavailable document back to an older available release, while HUP, INT, and TERM roll back an uncommitted replacement and terminate non-zero. Unavailable metadata sets both the latest APK URL and checksum URL to `null`. After that fail-closed metadata transition, the lane moves every public Latest APK/checksum, any interrupted-publication Latest staging/backup files, and each release directory named by the comma-separated `SECPAL_ANDROID_WITHDRAW_VERSIONS` value into a recoverable quarantine outside the public artifact root. The quarantine defaults to the sibling `android-artifacts-withdrawn` directory; set an absolute `SECPAL_ANDROID_DIRECT_WITHDRAWAL_ROOT` to use another non-public path. Before changing metadata, the lane resolves both roots on the SSH host, rejects containment or symlink traversal into the public tree, and refuses to overwrite an existing quarantine transaction. The lane requires only the configured SSH release host and does not load or synthesize signing credentials.
