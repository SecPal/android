...
When no supported signed replacement is available, `fastlane android withdraw_direct_apks` first recovers any interrupted metadata replacement and then changes the Stable channel, its `/android/` aliases, and the Beta channel metadata to `release_available: false`. Unavailable metadata sets both the latest APK URL and checksum URL to `null`. After that fail-closed metadata transition, the lane moves every public Latest APK/checksum and each release directory named by the comma-separated `SECPAL_ANDROID_WITHDRAW_VERSIONS` value into a recoverable quarantine outside the public artifact root. The quarantine defaults to the sibling `android-artifacts-withdrawn` directory; set `SECPAL_ANDROID_DIRECT_WITHDRAWAL_ROOT` to use another non-public path. The lane requires only the configured SSH release host and does not load or synthesize signing credentials.
