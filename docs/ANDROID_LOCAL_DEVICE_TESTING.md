...
- The exported Samsung receiver fails closed before Android 14, where Android cannot expose the original broadcast sender. On Android 14 and later, the sender UID must hold Samsung's `KNOX_CUSTOM_SETTING` or `KNOX_CUSTOM_SYSTEM` permission in addition to the existing action and managed-owner checks.
