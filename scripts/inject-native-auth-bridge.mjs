...
(function () {
  ...
  const nativeAuthSessionInvalidatedEventName =
  ...
  const installNativeAuthLifecycleListener = () => {
    ...
    const handleOrPromises = [plugin.addListener(
      ...
      nativeAuthSessionInvalidatedEventName,
      (payload) => {
        if (!payload || typeof payload !== "object") {
          return;
        }
        setAuthActive(false);
        invalidateScheduledNativeFetches("SESSION_INVALIDATED");
    ...
    const removeLifecycleListener = (event) => {
      ...
      const handles = Array.isArray(authState.nativeAuthLifecycleHandles)
        ? authState.nativeAuthLifecycleHandles
        : [];
      authState.nativeAuthLifecycleHandles = [];
    ...
    authState.nativeAuthLifecycleHandles = [];
    for (const handleOrPromise of handleOrPromises) {
      Promise.resolve(handleOrPromise).then((handle) => {
        if (authState.nativeAuthLifecycleDetached) {
          return Promise.resolve(handle?.remove?.()).catch(() => {});
        }
        if (handle) {
          authState.nativeAuthLifecycleHandles.push(handle);
        }
      })
      .catch(() => {
        // The remaining listener retains its independently registered handle.
      });
    }
  };
