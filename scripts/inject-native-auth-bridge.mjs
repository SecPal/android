...
const BRIDGE_ASSET_NAME_PATTERN =
  /^secpal-native-auth-bridge\.[0-9a-f]{64}\.js$/u;
const BRIDGE_SOURCE_PATH_PATTERN =
  /^\/secpal-native-auth-bridge\.[0-9a-f]{64}\.js$/u;
...
export function readApiBaseUrlFromStringsXml(stringsXml) {
  ...
  if (!apiBaseUrl) {
    throw new Error(
      "Android strings.xml contains an empty api_base_url string resource"
    );
  }
...
export function buildNativeAuthBridgeAsset(apiBaseUrl) {
  const content = buildNativeAuthBridgeBootstrapScript(apiBaseUrl);
  const digest = createHash("sha256").update(content, "utf8").digest("hex");
  const fileName = `secpal-native-auth-bridge.${digest}.js`;
...
function findExistingBridgeScriptRanges(html) {
  const ranges = [];
  ...
  while (pending.length > 0) {
    ...
    if (
      ...
      if (!location?.startTag || !location.endTag) {
        throw new Error(
          "Android web index contains an unterminated native auth bridge tag."
        );
      }
      const trailingLineBreak = html.startsWith(
        "\r\n",
        location.endTag.endOffset
      )
        ? 2
        : html[location.endTag.endOffset] === "\n"
          ? 1
          : 0;
      ranges.push({
        endOffset: location.endTag.endOffset + trailingLineBreak,
...
export function injectNativeAuthBridgeBootstrap(html, sourcePath) {
  if (!BRIDGE_SOURCE_PATH_PATTERN.test(sourcePath)) {
    throw new Error(
      "Native auth bridge source must be a controlled root-relative SHA-256 asset path."
    );
  }

  assertCompleteAndroidWebApplicationShell(html);
  const scriptTag = `<script id="${BOOTSTRAP_SCRIPT_ID}" src="${sourcePath}"></script>`;
  const existingRanges = findExistingBridgeScriptRanges(html).sort(
    (left, right) => right.startOffset - left.startOffset
  );
  let withoutExistingBridge = html;

  for (const range of existingRanges) {
    withoutExistingBridge =
      withoutExistingBridge.slice(0, range.startOffset) +
      withoutExistingBridge.slice(range.endOffset);
  }

  const shell = inspectAndroidWebApplicationShell(withoutExistingBridge);
  if (shell.moduleEntryStartOffset === null) {
    throw new Error(
      "Android web index must contain an external module entry script."
    );
  }

  return `${withoutExistingBridge.slice(0, shell.moduleEntryStartOffset)}${scriptTag}\n${withoutExistingBridge.slice(shell.moduleEntryStartOffset)}`;
}
...
function removeStaleBridgeAssets(assetRoot, currentFileName) {
  for (const entry of readdirSync(assetRoot, { withFileTypes: true })) {
    if (
      entry.isFile() &&
      entry.name !== currentFileName &&
      BRIDGE_ASSET_NAME_PATTERN.test(entry.name)
    ) {
...
export function injectNativeAuthBridgeIntoFile(indexHtmlPath, stringsXmlPath) {
  ...
  const stringsXml = readFileSync(resolve(stringsXmlPath), "utf8");
  assertCompleteAndroidWebApplicationShell(html, resolvedIndexHtmlPath);
  const apiBaseUrl = readApiBaseUrlFromStringsXml(stringsXml);
  const bridgeAsset = buildNativeAuthBridgeAsset(apiBaseUrl);
  const assetRoot = dirname(resolvedIndexHtmlPath);
  const bridgeAssetPath = join(assetRoot, bridgeAsset.fileName);
  const injectedHtml = injectNativeAuthBridgeBootstrap(
    html,
    bridgeAsset.sourcePath
  );

  writeFileAtomicallyIfChanged(bridgeAssetPath, bridgeAsset.content);
  writeFileAtomicallyIfChanged(resolvedIndexHtmlPath, injectedHtml);
  removeStaleBridgeAssets(assetRoot, bridgeAsset.fileName);

  return {
    bridgeAssetPath,
    bridgeFileName: bridgeAsset.fileName,
  };
}
