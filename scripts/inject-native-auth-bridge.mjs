...
export function buildNativeAuthBridgeBootstrapScript(apiBaseUrl) {
  ...
  return [
    ...
    "    bridge.loginWithPasskey = async (options) => {",
    "      const result = await getPlugin().loginWithPasskey(options ?? {});",
    "      authState.active = true;",
    "      return result;",
    "    };",
    "  }",
    "  if (typeof getPlugin().isVaultDeviceBoundWrapperAvailable === 'function' && typeof getPlugin().wrapVaultRootKey === 'function' && typeof getPlugin().unwrapVaultRootKey === 'function') {",
    ...
    "      }",
    "      try {",
    "        const currentUrl = new URL(location.href ?? apiOrigin, apiOrigin);",
