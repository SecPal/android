...
export function buildNativeAuthBridgeBootstrapScript(apiBaseUrl) {
  ...
  return [
    ...
    "      try {",
    "        const currentUrl = new URL(globalThis.location?.href ?? apiOrigin, apiOrigin);",
    "        if (currentUrl.pathname === pathname) {",
    "          return;",
    "        }",
    "        globalThis.location.href = new URL(pathname, currentUrl.href).toString();",
    "      } catch {",
    "        globalThis.location.href = pathname;",
    "      }",
    "    };",
    "    enterpriseBridge.addHardwareButtonListener((event) => {",
    "      if (event?.origin === 'samsung_knox_broadcast') {",
    "        openRoute('/profile');",
    "      }",
    "    });",
